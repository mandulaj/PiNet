...
Db.prototype.listSockets = function (cb) {
  var self = this;
  self.db.serialize(function(){
    self.db.all("SELECT sockets.id, sockets.userId, sockets.login, users.username, users.access FROM sockets JOIN users ON users.id = sockets.userId ORDER BY users.username, sockets.id", function(err, data){
      if (err) return cb(err, null);
      return cb(null, data);
    });
  });
