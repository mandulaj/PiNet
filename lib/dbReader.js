...
Db.prototype.createNewUser = function(data, cb) {
  ...
  createPassword(data.password, function(err, hash) {
    ...
    self.db.serialize(function() {
      ...
      self.db.run("INSERT INTO users (username, password, access, lastLogin) VALUES ((?), (?), (?), (?))", data.username, data.password, data.access, now, function(err, data) {
...
Db.prototype.updateLogin = function(id, cb) {
  ...
  self.db.serialize(function() {
    var now = new Date().valueOf();
    self.db.run("UPDATE users SET lastLogin = (?) WHERE id=(?)", now.toString(), id, function(err, data) {
...
Db.prototype.changePassword = function(id, oldPassword, newPassword, cb) {
  ...
  self.verifyUser(id, oldPassword, function(err, success) {
    ...
    createPassword(newPassword, function(err, hash) {
      if (err) return cb(err, false);
      self.db.serialize(function() {
        self.db.run("UPDATE users SET password = (?) WHERE id=(?)", hash, id, function(err, data) {
          if (err) {
            return cb(err, false);
          }
          return cb(null, true);
...
Db.prototype.reportFailedLogin = function(ip, cb) {
  ...
  self.db.serialize(function() {
    self.db.get("SELECT id, lastDate, threat FROM logins WHERE ip = (?)", ip, function(err, data) {
      if (err) return cb(err);
      if (!data) {
        self.db.run("INSERT INTO logins (ip, lastDate) VALUES((?), (?))", ip, now, function(err, data) {
          return cb(err);
        });
      } else {
        ...
        self.db.run("UPDATE logins SET accessed = accessed + 1, lastDate = (?), threat = (?), banned = (?) WHERE id = (?)", now, threat, banned, data.id, function(err, data) {
          return cb(err);
        });
      }
    });
...
Db.prototype.addSocket = function(id, socketId, cb){
  var self = this;
  self.db.serialize(function(){
    self.db.run('INSERT INTO sockets (id, userId) VALUES ((?), (?))', socketId, id, function(err){
      return cb(err);
    });
  });
};
